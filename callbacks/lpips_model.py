...
1:36
# --------------------
# Perceptual loss (2D default)
# --------------------
class Perceptual_Loss(nn.Module):
    """
    For 2D inputs [B,C,H,W] uses LPIPS.
    Set is_3d=False for standard image training.
    """
    def __init__(self, is_3d: bool = False, sample_ratio: float = 1.0):
        super().__init__()
        self.is_3d = is_3d
        self.sample_ratio = sample_ratio
        if self.is_3d:
            raise ValueError("This 2D revision doesn't include a 3D perceptual backbone.")
        self.perceptual_model = LPIPS().eval()
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        if input.dim() != 4 or target.dim() != 4:
            raise ValueError(f"Perceptual_Loss (2D) expects [B,C,H,W], got {input.shape}, {target.shape}")
        B = input.size(0)
        if self.sample_ratio < 1.0:
            idx = torch.randperm(B, device=input.device)[:max(1, int(B * self.sample_ratio))]
            input = input[idx]
            target = target[idx]
        return self.perceptual_model(input, target).mean()