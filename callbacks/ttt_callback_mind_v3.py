# callbacks/ttt_callback_mind_v2.py
import copy
import contextlib
...
class TestTimeTrainingCallback(L.Callback):
    ...
    Train objective (inner loop):
      - k-space DC (normalized L1) on acquired samples (using center-slice mask for cascades).

    Validation metric (no grad, on-the-fly):
      - Build low-frequency pseudo-reference from masked k-space (keep central ky lines).
      - On a center crop (W*crop_frac_w x H*crop_frac_h), compute MIND for recon vs LF reference.
      - Metric = mean L1 distance between MIND descriptors to MINIMIZE.

    Early-stop modes:
      - "first_drop": stop as soon as judged_metric > best*(1+min_delta_rel) + min_delta_abs.
      - "robust"   : judged_metric uses EMA(optional); require consecutive worsens >= patience.
    """
    ...
    def setup(self, trainer, pl_module, stage: Optional[str] = None):
        if self.enable_wandb_logging:
            ...
            if self.wandb_logger is None:
                print("[TTT-MIND] WARNING: WandbLogger not found but enable_wandb_logging=True. Proceeding without W&B logging.")
        else:
            print("[TTT-MIND] W&B logging disabled by enable_wandb_logging=False")
    ...
    def _log_wandb(self, trainer, payload: Dict[str, Any]):
        if trainer.is_global_zero and self.enable_wandb_logging and self.wandb_logger is not None:
            self.wandb_logger.experiment.log(payload)
    ...
    def on_predict_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx: int = 0):
        ...
        with torch.no_grad():
            ...
            self._log_wandb(trainer, {
                f"TTT/batch{batch_idx:03d}/mind_baseline": best_metric_raw,
        ...
        for it in range(self.inner_steps):
            ...
            if (it % self.log_every_n_steps == 0) or (it == self.inner_steps - 1):
                self._log_wandb(trainer, {
                    f"TTT/batch{batch_idx:03d}/loss_dc": float(loss_dc.item()),
                    f"TTT/batch{batch_idx:03d}/mind_cur_raw": metric_cur_raw,
                    f"TTT/batch{batch_idx:03d}/mind_cur_judged": metric_for_judge,
                    f"TTT/batch{batch_idx:03d}/mind_best": best_metric_judged,
        ...
        self._log_wandb(trainer, {
            f"TTT/batch{batch_idx:03d}/mind_final_best": best_metric_judged,
            f"TTT/batch{batch_idx:03d}/mind_final_best_raw": best_metric_raw,
            f"TTT/batch{batch_idx:03d}/it_when_best": it_when_best,
            f"TTT/batch{batch_idx:03d}/fname": fname,
            f"TTT/batch{batch_idx:03d}/slice_num": slice_num,
        })
