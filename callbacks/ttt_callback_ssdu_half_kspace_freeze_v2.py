# callbacks/ttt_callback_ssdu_half_kspace_freeze_v2.py
import copy
...
class TestTimeTrainingCallback(L.Callback):
    ...
    **V2: Flexible parameter freezing with both directions**
    - train_first_n_cascades: Train ONLY first N cascades (freeze the rest)
    - freeze_first_n_cascades: Freeze first N cascades (train the rest)
    - freeze_sens_net: Freeze sensitivity estimation network
    - freeze_prompts: Freeze all prompt modules in cascades
    
    Note: train_first_n_cascades and freeze_first_n_cascades are mutually exclusive.
          If both are set, train_first_n_cascades takes priority.
    """
    ...
    def __init__(
        ...
        if self.train_first_n_cascades > 0 and self.freeze_first_n_cascades > 0:
            print("[TTT-V2] WARNING: Both train_first_n_cascades and freeze_first_n_cascades are set.")
            print("[TTT-V2]          train_first_n_cascades takes priority, ignoring freeze_first_n_cascades.")
    ...
    def setup(self, trainer, pl_module, stage: Optional[str] = None):
        if self.enable_wandb_logging:
            ...
            if self.wandb_logger is None:
                print("[TTT-V2] WARNING: WandbLogger not found but enable_wandb_logging=True. Proceeding without W&B logging.")
        else:
            print("[TTT-V2] W&B logging disabled by enable_wandb_logging=False")
    ...
    def on_predict_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx: int = 0):
        ...
        if self.train_first_n_cascades > 0:
            cascade_mode = f"train first {self.train_first_n_cascades} cascades (freeze rest)"
        elif self.freeze_first_n_cascades > 0:
            cascade_mode = f"freeze first {self.freeze_first_n_cascades} cascades (train rest)"
        else:
            cascade_mode = "all cascades trainable"
        
        # Print freezing summary
        print(f"[{device_name}] 🔒 Freezing Strategy Applied (V2):")
        print(f"  - Cascade strategy: {cascade_mode}")
        print(f"  - Freeze sens_net: {'✓' if self.freeze_sens_net else '✗'}")
        print(f"  - Freeze prompts: {'✓' if self.freeze_prompts else '✗'}")
