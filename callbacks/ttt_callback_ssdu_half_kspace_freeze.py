# callbacks/ttt_callback_ssdu_half_kspace_freeze.py
import copy
...
class TestTimeTrainingCallback(L.Callback):
    ...
    **NEW: Flexible parameter freezing options**
    - freeze_first_n_cascades: Freeze first N cascades (0-indexed)
    ...
    def __init__(
        ...
        # ---- NEW: Freezing options ----
        freeze_first_n_cascades: int = 0,      # Freeze first N cascades (0 = no freezing)
    ...
    def _apply_freezing_strategy(self, model: torch.nn.Module) -> Tuple[int, int]:
        ...
        # Freeze first N cascades if requested
        if self.freeze_first_n_cascades > 0 and hasattr(model, 'promptmr') and hasattr(model.promptmr, 'cascades'):
    ...
    def setup(self, trainer, pl_module, stage: Optional[str] = None):
        if self.enable_wandb_logging:
            ...
            if self.wandb_logger is None:
                print("[TTT-Freeze] WARNING: WandbLogger not found but enable_wandb_logging=True. Proceeding without W&B logging.")
        else:
            print("[TTT-Freeze] W&B logging disabled by enable_wandb_logging=False")
    ...
    def on_predict_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx: int = 0):
        ...
        print(f"[{device_name}] 🔒 Freezing Strategy Applied:")
        print(f"  - Freeze first {self.freeze_first_n_cascades} cascades: {'✓' if self.freeze_first_n_cascades > 0 else '✗'}")
