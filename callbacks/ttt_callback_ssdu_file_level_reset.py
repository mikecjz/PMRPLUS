# callbacks/ttt_callback_ssdu_file_level_reset.py
"""
Test-Time Training with FILE-LEVEL reset strategy.

Key Feature:
- Model is reset to initial checkpoint state when processing a NEW file/case
- Within the SAME file, slices maintain continuous adaptation (no reset between slices)
- This allows slice-to-slice knowledge transfer while ensuring independence between cases
"""
import copy
...
class TestTimeTrainingCallback(L.Callback):
    ...
    **FILE-LEVEL RESET**: Model resets only when a new file is detected; 
                          slices within same file maintain adapted state.
    """
    ...
    def __init__(
        ...
        # ---- File-level reset tracking ----
        self.current_fname: Optional[str] = None  # Track current file being processed
        self._initial_state: Optional[dict] = None  # Initial checkpoint state (saved once)
    ...
    def setup(self, trainer, pl_module, stage: Optional[str] = None):
        ...
        if self._true_initial_state is None:
            ...
            print(f"[TTT-FileLevelReset] 🔵 Saved TRUE initial checkpoint state in setup")
        ...
        if self.enable_wandb_logging:
            ...
            if self.wandb_logger is None:
                print("[TTT-FileLevelReset] WARNING: WandbLogger not found but enable_wandb_logging=True. Proceeding without W&B logging.")
        else:
            print("[TTT-FileLevelReset] W&B logging disabled by enable_wandb_logging=False")
    ...
    def on_predict_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx: int = 0):
        ...
        # ========== FILE-LEVEL RESET LOGIC ==========
        # Save initial state on first batch (for backward compatibility)
        if self._initial_state is None:
            self._initial_state = copy.deepcopy(pl_module.state_dict())
            self.current_fname = fname
            self.slice_count = 0
            print(f"[{device_name}] 🔵 Saved initial checkpoint state")
        
        # Detect file change -> reset to TRUE initial state (方案2)
        if fname != self.current_fname:
            ...
            if self._true_initial_state is not None:
                ...
                # 回退到原来的逻辑
                pl_module.load_state_dict(self._initial_state)
                print(f"[{device_name}] ⚠️ Fallback to old reset logic")
            self.current_fname = fname
            self.slice_count = 0
        ...
        # Create working copy from CURRENT pl_module state (which may be adapted from previous slice)
        ...
        with torch.no_grad():
            ...
            if trainer.is_global_zero and self.enable_wandb_logging and self.wandb_logger is not None:
                self.wandb_logger.experiment.log({
                    ...
                    "TTT/inner_iter": -1,
                    f"TTT_file/{fname}/slice_count": self.slice_count,
                })
        ...
        for it in range(self.inner_steps):
            ...
            if trainer.is_global_zero and self.enable_wandb_logging and self.wandb_logger is not None and \
               (it % self.log_every_n_steps == 0 or it == self.inner_steps - 1):
                key = f"TTT/{fname}/slice{slice_num:03d}"
                self.wandb_logger.experiment.log({
                    ...
                    "TTT/global_step": int(batch_idx * self.inner_steps + it),
                })
        ...
        # ========== KEY CHANGE: Load best state into pl_module (no reset after batch) ==========
        ...
        print(f"[{device_name}] 🔗 Model state preserved for next slice in same file")
        # =======================================================================================
        ...
        if trainer.is_global_zero and self.enable_wandb_logging and self.wandb_logger is not None:
            ...
            self.wandb_logger.experiment.log({
                ...
                f"{key}/slice_in_file": self.slice_count,
            })

    @rank_zero_only
    def on_predict_batch_end(self, trainer, pl_module, prediction, batch, dataloader_idx: int = 0, **kwargs):
        # ========== KEY CHANGE: No reset after batch - model state persists ==========
        # The model state from TTT is preserved for the next slice in the same file
        pass
