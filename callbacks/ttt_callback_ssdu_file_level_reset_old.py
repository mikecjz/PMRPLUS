...
class TestTimeTrainingCallback(L.Callback):
    ...
    **FILE-LEVEL RESET**: Model resets only when a new file is detected; 
    ...
    def on_predict_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx: int = 0):
        ...
        # Save initial state on first batch
        ...
        # Detect file change -> reset to initial state
        if fname != self.current_fname:
            print(f"[{device_name}] 🔄 New file detected: '{self.current_fname}' -> '{fname}'")
            print(f"[{device_name}] 🔄 Resetting model to initial checkpoint state (processed {self.slice_count} slices in previous file)")
            pl_module.load_state_dict(self._initial_state)
            # 强制重新加载原始checkpoint权重，确保完全重置
